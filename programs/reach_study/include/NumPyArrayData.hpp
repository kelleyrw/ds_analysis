#ifndef NUMPYARRAYDATA_HPP
#define NUMPYARRAYDATA_HPP
...
template<typename T> class NumPyArrayData
{
    ...
    NumPyArrayData<T>(const np::ndarray &arr)
    {
        ...
        {
            std::stringstream ss;
            ss << "NumPyArrayData: Unexpected data type ("
               << bp::extract<const char*>(dtype.attr("__str__")()) << ") received. ";
